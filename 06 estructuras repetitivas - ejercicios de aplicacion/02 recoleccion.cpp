...
c) El piso término que tuvo la mayor cantidad de toneladas recolectadas en el 
...
int main() {
    ...
    while (control < 4) {
		...
		//Para la primera iteración, se inicializan los valores del mejor y peor trimestre
		//Con la información que introdujo el usuario
        if( band_primera_iter == true) {
            mejor_trim_cant_recol = total_trim;
            peor_trim_cant_recol = total_trim;
            band_primera_iter = false;
        } else {
        	// Se realiza la comparación del total del trimestre para verificar si fue
        	// mejor que la que ya se tenía identificada
            if( total_trim > mejor_trim_cant_recol) {
				...
				// Se realiza la comparación del total del trimestre para verificar si fue
        		// peor que la que ya se tenía identificada
				if (total_trim < peor_trim_cant_recol) {
                	peor_trim = control + 1;
                	peor_trim_cant_recol = total_trim;
            	}
        }
		// Condición para verificar cuál fue el piso térmico que tuvo la mayor
		//recolección en el tercer trimestre
        if(control == 3) {
